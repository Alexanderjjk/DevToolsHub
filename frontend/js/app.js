...
function initHeader() {
    ...
    // Window resize handles (frameless window edge resizing)
    initWindowResize();
}
...
// ============================================================
// WINDOW RESIZE — Drag edges/corners to resize frameless window
// Supports west/north resizes with simultaneous move (anchored opposite edge)
// ============================================================
function initWindowResize() {
    const handles = document.querySelectorAll('.window-resize-handle');
    ...
    handles.forEach(handle => {
        handle.addEventListener('mousedown', (e) => {
            ...
            // Get initial window position from pywebview
            const a = api();
            if (a && a.get_window_position) {
                a.get_window_position().then(function(pos) {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        // Use rAF for smoother updates (vs throttle)
        if (_resizeFramePending) return;
        _resizeFramePending = true;
        _resizeRafId = requestAnimationFrame(function() {
            _resizeFramePending = false;
            _doResize(e.screenX, e.screenY);
        });
    });

    function _doResize(mouseX, mouseY) {
        ...
        if (needsMove && a.resize_and_move_window) {
            // Atomic resize + move for west/north edges
            try { a.resize_and_move_window(Math.round(newW), Math.round(newH), Math.round(newX), Math.round(newY)); } catch(ex) {}
        } else if (a.resize_window) {
            // Simple resize for east/south edges
            try { a.resize_window(Math.round(newW), Math.round(newH)); } catch(ex) {}
        }
    }
...
// ============================================================
function initTheme() {
    // Restore dark/light mode — try DB first, then localStorage fallback
    // DB is the source of truth (persists across reinstall/relocations)
    // localStorage is the instant fallback (before API is ready)
    ...
    // Once API is ready, load theme from DB (source of truth)
    waitForApi(5000).then(() => {
        var a = api();
        if (a && a.get_setting) {
            a.get_setting('theme_mode').then(function(dbTheme) {
                if (dbTheme && (dbTheme === 'light' || dbTheme === 'dark')) {
                    ...
                    localStorage.setItem('devtools-theme-mode', dbTheme);
                    updateThemeModeUI();
                } else if (!dbTheme) {
                    // No DB setting yet — save current state
                    a.set_setting('theme_mode', savedThemeMode);
                }
            }).catch(function() {});
        }
    }).catch(function() {});
...
function toggleThemeMode() {
    ...
    // Save to both localStorage (instant) and DB (persistent)
    localStorage.setItem('devtools-theme-mode', newMode);
    updateThemeModeUI();

    var a = api();
    if (a && a.set_setting) {
        a.set_setting('theme_mode', newMode).catch(function() {});
    }
